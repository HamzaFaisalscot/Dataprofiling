...
async def generate_metadata(request: MetadataRequest):
    try:
        ...
        try:
            ...
            print("✅ Keys under 'cleaned/':", keys)
        except Exception as e:
            print("⚠️ list_objects_v2 error:", e)
        ...
        try:
            ...
            raise HTTPException(status_code=404, detail=f"S3 CSV read error: {err.get('Code')} - {err.get('Message')}")

        # Attempt to fetch data profile JSON, but continue if missing
        try:
            # Note folder corrected from 'profile/' to 'profiled/'
            profile_obj = s3.get_object(
                Bucket=settings.S3_BUCKET_NAME,
                Key=f"profiled/{request.file_id}.json"
            )
            profile_content = profile_obj['Body'].read().decode('utf-8')
            profile_data = json.loads(profile_content)
        except ClientError as e:
            err = e.response['Error']
            if err.get('Code') == 'NoSuchKey':
                print(f"Profile not found for {request.file_id}, proceeding without profile metadata.")
                profile_data = {}
            else:
                raise HTTPException(status_code=404, detail=f"S3 profile read error: {err.get('Code')} - {err.get('Message')}")

        # Extract column names from CSV header and profile
        reader = csv.reader(csv_content.splitlines())
        header = next(reader, [])
        columns = []
        for col in header:
            col_meta = profile_data.get('fields', {}).get(col, {})
            dtype = col_meta.get('type', 'STRING')
            nullable = col_meta.get('nullable', True)
            tag = col_meta.get('tag')
            glossary = col_meta.get('glossary_term')
            columns.append({
                'name': col,
                'type': dtype,
                'nullable': nullable,
        ...
        # Build prompt dynamically for Bedrock
        prompt_payload = {
            "dataset": request.file_id,
            "columns": columns,
            "csv_sample": csv_content[:2000]
        }
        prompt = (
                "Generate a valid, compact JSON object with enriched metadata for the dataset. "
                "Return only the JSON object, with no explanations, markdown formatting, or code blocks. "
                "The JSON should include the dataset ID and a list of columns, each with 'name', 'type', 'description', 'tags', and 'glossary_terms'.\n\n"
                "Here is the dataset:\n"
        ...
        # Call Bedrock to enrich metadata
        #response = bedrock.invoke_model(
        #    modelId="meta.llama3-70b-instruct-v1:0",
        #    body=json.dumps({
        #        "prompt": (
        #
        #    "Given the dataset '" + request.file_id + "' with these columns and sample data, "
        #            "Generate a valid, compact JSON object with enriched metadata for the dataset. Return only the JSON object, with no explanations, markdown formatting, or code blocks. The JSON should include the dataset ID and a list of columns, each with 'name', 'type', 'description', 'tags', and 'glossary_terms'."
        #            "If profile metadata is unavailable, infer from data. "
        ...
        print(f"generate_metadata error: {e}")
